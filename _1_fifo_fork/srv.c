...
void send(int fd_file, int fd_fifo, char * buf)
{
	int size = 0;
	while(1)
		{
			...
			{
				printf("SRV: in send function: Read error\n");
				exit(errno);
			}
			...
			{
				printf("SRV: in send function: Write error\n");
				exit(errno);
			}
		}
}
...
int main(int argc, char *argv[], char *envp[])
{
	...
	*/
		...
		if ((pid_fork = fork()) < 0)
		{
			printf("SRV: in main Fork error\n");
			exit(errno);
		}
		else if (pid_fork == 0)
		{
			// Child
			send(fd_file, fd_fifo, buf);
			exit(0);
		}
		else
		{
